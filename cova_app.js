...
const D3Chart = {  
  ...
  mounted() {
    this.$nextTick(() => {
      if (this.graph['json']) {
        const jsonData = JSON.parse(this.graph.json);
        this.drawChart(this.graph.id, jsonData);
      }
    });
  },
  ...
  methods: {
    drawChart(chartId, data) {
      // Using the 'data' object to render your chart
      // Number of deaths chart

      var dates = data.results.date.map(function(d) { return new Date(d); });
      var cumDeaths = data.results.cum_deaths.map(Number);
      ...
      // Set up the dimensions and margins of the graph
        ...
        var y = d3.scaleLinear().range([height, 0]).domain([0, d3.max(cumDeaths)]);
...
// Replacing 'plotly-chart' with 'd3-chart' in Vue instance
var vm = new Vue({
  el: '#app',
  components: {
    'd3-chart': D3Chart,
...
// const PlotlyChart = {                           //This is a Vue component for rendering Plotly charts. It takes a prop called 'graph' and renders a div with the specified ID.
//     props: ['graph'],
//     render(h) {
//         return h('div', {
//             attrs: {
...
//     mounted() {                                     //The mounted and updated lifecycle hooks are used to interact with the Plotly library. When the component is mounted or updated, it parses JSON data from the prop, makes it responsive, and updates the Plotly chart.
...
// var vm = new Vue({                               //Vue Instance Initialization:
//     el: '#app',                                  //This specifies the HTML element with the id "app" to which Vue will be attached. Vue will control and manage the content inside this element.
